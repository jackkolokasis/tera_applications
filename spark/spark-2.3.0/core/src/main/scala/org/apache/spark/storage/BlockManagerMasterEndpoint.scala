...
class BlockManagerMasterEndpoint(
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    case RegisterBlockManager(blockManagerId, maxOnHeapMemSize, maxOffHeapMemSize,
      pmemMaxOffHeapMemory, slaveEndpoint) =>
      context.reply(register(blockManagerId, maxOnHeapMemSize, maxOffHeapMemSize,
        pmemMaxOffHeapMemory, slaveEndpoint))
  ...
  private def register(
    ...
    if (!blockManagerInfo.contains(id)) {
      ...
      logInfo("Registering block manager %s with %s RAM, %s".format(
        id.hostPort,
        Utils.bytesToString(maxOnHeapMemSize + maxOffHeapMemSize + maxPmemOffHeapMemSize), id))

      blockManagerIdByExecutor(id.executorId) = id

      blockManagerInfo(id) = new BlockManagerInfo(
        id, System.currentTimeMillis(), maxOnHeapMemSize, maxOffHeapMemSize, maxPmemOffHeapMemSize,
        slaveEndpoint)
    }
    listenerBus.post(SparkListenerBlockManagerAdded(time, id,
      maxOnHeapMemSize + maxOffHeapMemSize + maxPmemOffHeapMemSize, Some(maxOnHeapMemSize),
      Some(maxOffHeapMemSize), Some(maxPmemOffHeapMemSize)))
    id
  }
