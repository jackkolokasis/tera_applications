...
import org.apache.spark.internal.config.{STORAGE_UNROLL_MEMORY_THRESHOLD, UNROLL_MEMORY_CHECK_PERIOD, UNROLL_MEMORY_GROWTH_FACTOR, TERAHEAP_ENABLED}
...
private[spark] class MemoryStore(
  ...
  private def putIterator[T](
    ...
    if (keepUnrolling) {
      val entryBuilder = valuesHolder.getBuilder(blockId, conf)
...
private trait ValuesHolder[T] {
   ...
   *         size.
   */
  def getBuilder(blockId: BlockId, conf: SparkConf): MemoryEntryBuilder[T]
}
...
private class DeserializedValuesHolder[T](
  ...
  override def getBuilder(blockId: BlockId, conf: SparkConf): MemoryEntryBuilder[T] = new MemoryEntryBuilder[T] {
    // We successfully unrolled the entirety of this block
    arrayValues = vector.toArray
    // Mark the cached objects to be moved to H2 (TeraHeap) 
    if (conf.get(TERAHEAP_ENABLED)) {
      if (blockId.isRDD) {
        val rddId = blockId.toString().split('_')(1).toLong
        val partId = blockId.toString().split('_')(2).toLong

        Platform.h2MarkAndMoveObj(arrayValues, rddId, partId);
      }
    }
    vector = null
