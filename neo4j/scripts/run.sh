...
create_database_if_not_exist() {
  # Check if the database already exists. If it exists just return,
  # otherwise create it
  if [ -d "${MNT_DATABASE}/intermediate/${GRAPH_NAME}/data" ]
  then
    return
  fi
  ...
  if grep -q "weight" "${DATASET}/${GRAPH_NAME}.properties"
  then
    is_weighted="true"
  fi
  ...
  ./load-graph.sh \
    ...
    --input-vertex-path "${DATASET}/${GRAPH_NAME}.v" \
    --input-edge-path "${DATASET}/${GRAPH_NAME}.e" \
    --output-path "${MNT_DATABASE}/intermediate/${GRAPH_NAME}" \
    --weighted "${is_weighted}"

  cd - > /dev/null || exit

  cd "${MNT_DATABASE}/intermediate/${GRAPH_NAME}" || exit
  mv database data
  cd - >/dev/null || exit 
}
