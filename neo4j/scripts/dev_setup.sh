...
destroy_th() {
		...
		rm -rf "${NEO4J_DB_DIR:?}"/*
		retValue=$?
		message="Remove neo4j database directory" 
		check ${retValue} "${message}"
		...
		sudo umount "${NEO4J_DB_DIR}"
		retValue=$?
		message="Unmount $DEV_NEO4J_DB" 
		check ${retValue} "${message}"
}
...
if [ $TC ]
then
	if ! mountpoint -q "${NEO4J_DB_DIR}"
	then
		# Setup disk for zookeeper
		sudo mount /dev/"${DEV_NEO4J_DB}" "${NEO4J_DB_DIR}"
		# Check if the command executed succesfully
		retValue=$?
		message="Mount ${DEV_NEO4J_DB} for zookeeper" 
		check ${retValue} "${message}"

		sudo chown kolokasis "${NEO4J_DB_DIR}"
		# Check if the command executed succesfully
		retValue=$?
  ...
  cd - > /dev/null || exit
	...
	then
		...
		# Check if the command executed succesfully
		retValue=$?
		message="Change ownerships /mnt/${NEO4J_DB_DIR}" 
		check ${retValue} "${message}"
	fi
fi
exit
